...
class Visualizer {
  ...
  async sendPacket(from, to, label, data, isMalicious = false) {
    return new Promise((resolve) => {
      ...
      // Log it
      ui.logFlow(
        `[${from.toUpperCase()} -> ${to.toUpperCase()}] ${label}`,
        isMalicious ? "error" : "step",
      );
...
const ui = {
  logFlow(msg, type = "info") {
    ...
    if (
      document
        .querySelector('.tab[data-tab="flow-log"]')
  ...
  showPacketData(data) {
    ...
    if (
      document
        .querySelector('.tab[data-tab="packet-data"]')
        .classList.contains("active")
    ) {
    ...
    steps: [
      {
        label: "Login Request",
        action: async (v) =>
          await v.sendPacket("client", "server", "POST /login", {
            username: "user",
            password: "***",
          }),
      },
      {
        ...
        action: async (v) => {
          await v.sendPacket("server", "db", "Query User", {
            sql: "SELECT * FROM users WHERE...",
          });
          await v.sendPacket("db", "server", "User Found", {
            id: 1,
            role: "user",
          });
        },
      },
      {
        ...
        action: async (v) => {
          ...
          await v.sendPacket("server", "db", "Store Session", {
            sid: "sess_123abc",
            uid: 1,
          });
        },
      },
      {
        label: "Set-Cookie",
        action: async (v) => {
          await v.sendPacket("server", "client", "200 OK", {
            "Set-Cookie": "sid=sess_123abc; HttpOnly",
          });
      ...
      },
      {
        ...
        action: async (v) => {
          await v.sendPacket("client", "server", "GET /dashboard", {
            headers: { Cookie: "sid=sess_123abc" },
          });
          // Server checks DB
          await v.sendPacket("server", "db", "Validate Session", {
            sid: "sess_123abc",
          });
          await v.sendPacket("db", "server", "Valid", { uid: 1 });
          await v.sendPacket("server", "client", "200 OK (Data)", {
  ...
  jwt: {
    ...
    steps: [
      ...
      },
      {
        label: "Verify & Sign",
        action: async (v) => {
          await v.sendPacket("server", "db", "Check Creds", {
            sql: "SELECT...",
          });
      ...
      },
      {
        ...
        action: async (v) => {
          const token = "eyJhbG..sig";
          await v.sendPacket("client", "server", "GET /api/orders", {
  ...
  oauth: {
    ...
    steps: [
      ...
      },
      {
        ...
        action: async (v) => {
          await v.sendPacket("client", "auth", "Redirect /authorize", {
            client_id: "app_1",
            scope: "email",
          });
        },
      },
      {
        ...
        action: async (v) => {
          ...
          await v.sendPacket("auth", "client", "Callback /cb", {
            code: "auth_code_xyz",
          });
        },
      },
      {
        label: "Exchange Code",
        action: async (v) => {
          await v.sendPacket("client", "auth", "POST /token", {
            code: "auth_code_xyz",
            client_secret: "***",
          });
      ...
      },
      {
        ...
        action: async (v) => {
          await v.sendPacket("auth", "client", "200 OK", {
            access_token: "mw_token_007",
          });
      ...
      },
      {
        ...
        action: async (v) => {
          await v.sendPacket("client", "api", "GET /mails", {
            Authorization: "Bearer mw_token_007",
          });
      ...
      },
      {
        ...
        action: async (v) => {
          await v.sendPacket(
            ...
            "server",
            "GET /dashboard",
            { Cookie: "sid=sess_123abc" },
            true,
          );
          ui.logFlow(
            "Server cannot distinguish attacker from user caused by lack of CSRF/Rotation.",
            "error",
          );
          await v.sendPacket(
            "server",
  ...
  expiry: {
    ...
    steps: [
      ...
      },
      {
        ...
        action: async (v) => {
          await v.sendPacket("client", "server", "GET /api", {
            Auth: "Bearer jwt_exp_5min",
          });
...
function loadScenario(key, attackKey = null) {
  ...
  if (!attackKey) {
    ...
    document
      .querySelector(`[data-attack="${attackKey}"]`)
