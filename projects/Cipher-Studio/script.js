...
const outCaesar = document.getElementById('outCaesar');
...
// --- Event Listeners ---
plainTextInput.addEventListener('input', processCryptography);
shiftSlider.addEventListener('input', (e) => {
    shiftValue.innerText = e.target.value;
    processCryptography();
});

// --- Core Processing Function ---
async function processCryptography() {
    ...
    // 1. Base64
    outBase64.innerText = encodeBase64(text);

    // 2. Caesar Cipher
    const shift = parseInt(shiftSlider.value);
    outCaesar.innerText = encodeCaesar(text, shift);

    // 3. SHA-256 Hash (Asynchronous Web Crypto API)
    outSHA.innerText = await generateSHA256(text);
}
...
function encodeBase64(str) {
    try {
        // btoa handles ASCII perfectly. For unicode, we'd need a deeper escape function, 
        // but native btoa is fine for a standard sandbox.
...
function encodeCaesar(str, shift) {
    return str.replace(/[a-zA-Z]/g, function(char) {
        const base = char <= 'Z' ? 65 : 97; // 65 is 'A', 97 is 'a' in ASCII
