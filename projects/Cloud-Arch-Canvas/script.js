...
// Config
const TYPE_CONFIG = {
    'compute': { icon: '💻', color: '#3fb950' },
    'database': { icon: '🗄️', color: '#58a6ff' },
    'storage': { icon: '🪣', color: '#d29922' }
};
...
canvasWrapper.addEventListener('drop', (e) => {
    ...
    nodes.push({ id: nextNodeId++, type, x, y });
    draw();
});

// --- Canvas Interactions ---
function getNodeAt(x, y) {
    // Search in reverse to grab the top-most node
...
canvas.addEventListener('mousedown', (e) => {
    ...
    if (e.button === 2) {
        ...
        if (target) {
            ...
            connections = connections.filter(c => c.from !== target.id && c.to !== target.id);
            draw();
        }
        return;
    }

    selectedNode = getNodeAt(mx, my);
    ...
    if (selectedNode) {
        if (e.shiftKey) {
            ...
            isDragging = true;
            dragOffsetX = mx - selectedNode.x;
            dragOffsetY = my - selectedNode.y;
        }
    }
});
...
canvas.addEventListener('mousemove', (e) => {
    ...
    if (isDragging && selectedNode) {
        selectedNode.x = mousePos.x - dragOffsetX;
        selectedNode.y = mousePos.y - dragOffsetY;
        draw();
    } else if (isConnecting && selectedNode) {
        draw(); // Force redraw to show dynamic connecting line
    }
});

canvas.addEventListener('mouseup', () => {
    if (isConnecting && selectedNode) {
        const targetNode = getNodeAt(mousePos.x, mousePos.y);
        // Ensure we dropped on a different node, and connection doesn't already exist
        if (targetNode && targetNode.id !== selectedNode.id) {
            const exists = connections.find(c => 
                (c.from === selectedNode.id && c.to === targetNode.id) ||
                (c.to === selectedNode.id && c.from === targetNode.id)
            );
            if (!exists) {
                connections.push({ from: selectedNode.id, to: targetNode.id });
            }
        }
    }
...
function drawLines() {
    ...
    connections.forEach(conn => {
        ...
        if (n1 && n2) {
            ctx.beginPath();
            ctx.moveTo(n1.x + NODE_WIDTH / 2, n1.y + NODE_HEIGHT / 2);
            ctx.lineTo(n2.x + NODE_WIDTH / 2, n2.y + NODE_HEIGHT / 2);
            ctx.stroke();
        }
    });
    ...
    if (isConnecting && selectedNode) {
        ...
        ctx.beginPath();
        ctx.moveTo(selectedNode.x + NODE_WIDTH / 2, selectedNode.y + NODE_HEIGHT / 2);
...
function drawNodes() {
    nodes.forEach(node => {
        ...
        ctx.font = '14px Inter';
        ctx.fillText(`${config.icon} ${node.type.toUpperCase()}`, node.x + 10, node.y + 25);
    });
}
...
document.getElementById('btnExport').addEventListener('click', () => {
    const output = JSON.stringify({ nodes, connections }, null, 2);
