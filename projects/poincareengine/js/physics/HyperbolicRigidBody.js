...
export class HyperbolicRigidBody {
    constructor() {
        this.position = new Complex(0, 0);
        this.velocity = new Complex(0, 0);
        this.angle = 0;
        this.angularVelocity = 0;
    }

    integrate(dt) {
        let v_comp = this.velocity.scale(dt);
        let trans = Mobius.translation(this.position);
        this.position = trans.apply(v_comp);

        if (this.position.magSq() >= 0.998) {
            this.position = this.position.normalize().scale(0.999); // absolute boundary limit
        }
