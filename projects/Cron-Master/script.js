...
const nextDatesList = document.getElementById('nextDatesList');

// Run on load and on input
cronInput.addEventListener('input', processCron);
window.addEventListener('DOMContentLoaded', processCron);

function processCron() {
    const expr = cronInput.value.trim().replace(/\s+/g, ' ');
    const parts = expr.split(' ');

    if (parts.length !== 5) {
        showError("Invalid syntax. Expected 5 parts separated by spaces.");
        return;
    }

    try {
        const parsed = {
            min: parsePart(parts[0], 0, 59),
        ...
        humanReadable.innerText = `At minute ${formatList(parsed.min)}, past hour ${formatList(parsed.hour)}, on day of month ${formatList(parsed.dom)}, in month ${formatList(parsed.month)}, on day of week ${formatList(parsed.dow)}.`;

        calculateNextDates(parsed);
...
function showError(msg) {
    ...
    humanReadable.innerText = msg;
    nextDatesList.innerHTML = '';
}

function parsePart(str, min, max) {
    let allowed = [];
    
    // Handle wildcards and step values (e.g., * or */15)
    if (str.startsWith('*')) {
        let step = 1;
        if (str.includes('/')) {
            step = parseInt(str.split('/')[1]);
    ...
    const listParts = str.split(',');
    listParts.forEach(p => {
        // Handle ranges (e.g., 1-5)
        if (p.includes('-')) {
            const range = p.split('-');
            const rStart = parseInt(range[0]);
            const rEnd = parseInt(range[1]);
            if (isNaN(rStart) || isNaN(rEnd) || rStart < min || rEnd > max || rStart > rEnd) throw new Error("Invalid range.");
            for (let i = rStart; i <= rEnd; i++) allowed.push(i);
        } else {
            // Handle single numbers
            const num = parseInt(p);
            if (isNaN(num) || num < min || num > max) throw new Error(`Value ${p} out of bounds (${min}-${max}).`);
            allowed.push(num);
        }
    });
...
function formatList(arr) {
    if (arr.length > 20) return "every (*)";
    return arr.join(',');
}

function calculateNextDates(parsed) {
    nextDatesList.innerHTML = '';
    ...
    let matches = [];
    let iterations = 0;
    const MAX_ITERATIONS = 100000; // Failsafe to prevent infinite loops

    while (matches.length < 5 && iterations < MAX_ITERATIONS) {
        iterations++;
        
        let min = currentDate.getMinutes();
        let hr = currentDate.getHours();
        let dom = currentDate.getDate();
        let month = currentDate.getMonth() + 1; // 1-12
        let dow = currentDate.getDay();

        // Standard cron logic: if DOM and DOW are both restricted, it's an OR condition.
        // For simplicity in this visualizer, we check if current values exist in allowed arrays.
        let domMatch = parsed.dom.includes(dom);
        let dowMatch = parsed.dow.includes(dow);
        
        let dayMatch = false;
        if (parsed.dom.length === 31) dayMatch = dowMatch;
        else if (parsed.dow.length === 7) dayMatch = domMatch;
        else dayMatch = domMatch || dowMatch;

        if (
            parsed.month.includes(month) &&
            dayMatch &&
            parsed.hour.includes(hr) &&
            parsed.min.includes(min)
        ) {
            matches.push(new Date(currentDate));
        }
    ...
    if (matches.length === 0) {
        nextDatesList.innerHTML = '<li>No upcoming dates found (Infinite loop prevented).</li>';
        return;
    }
